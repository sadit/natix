...
{
	public class ApproxGraphOptRandomRestarts : BasicIndex
	{
		...
		public ApproxGraphOptRandomRestarts ()
		{
		}

		public ApproxGraphOptRandomRestarts(ApproxGraphOptRandomRestarts a) : base()
		{
			this.DB = a.DB;
			...
			while (prev > curr) {
				...
				for (int i = 0; i < window; ++i) {
					...
					if (expanded.Add(next)) {
						this.InternalSearch (q, next, expanded, res);
						foreach (var p in res) {
		...
		protected void InternalSearch(object q, int startID, HashSet<int> expanded, IResult res)
		{
