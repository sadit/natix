...
namespace natix.SimilaritySearch
{
	...
	public class BinH8Space : MetricDB
	{
		...
		public static IList<byte> ParseAndLoadFromFile (string name)
		{
			return BinQ8HammingSpace.LoadObjectFromFile(name, true);
		}
		
		/// <summary>
		/// Converts 'name' into an object
		/// </summary>
		public virtual object Parse (string name)
		{
			//Console.WriteLine ("Parsing '{0}', isquery: {1}", name, isquery);
			if (name.StartsWith ("obj")) {
				return this[int.Parse (name.Split (' ')[1])];
			}
			var res = ParseAndLoadFromFile (name);
			return res;
		}
