...
{
	public class InOrderUnbalancedTreeIntersection<T> : IIntersection<T> where T: IComparable
	{
		...
		public InOrderUnbalancedTreeIntersection (double splitAtRatio, ISearchAlgorithm<T> searchalg)
		{
		...
		public virtual IList<T> Intersection (IList< IList<T> > postings)
		{
			...
			List<T> res = new List<T> (1 + (postings [0].Count >> 1));
			List<T> tmp = null;
			List<T> swapAux = null;
