...
namespace natix.CompactDS
{
	...
	/// </summary>
	public class IEncoder32GenericIO
	{
		...
		public static IIEncoder32 Load (BinaryReader Input)
		{
			byte idType = Input.ReadByte ();
			var type = Catalog[idType];
			if (type == null) {
				var s = String.Format ("IEncoder32GenericIO.Catalog returned null using idType: {0}," +
					"is it deprecated?", idType);
				throw new ArgumentNullException (s);
			}
			var coder = (IIEncoder32)Activator.CreateInstance (type);
			coder.Load (Input);
			return coder;
		}
